...
// Define the shape of individual recommendation issues
interface FeedbackItem {
  issue: string;
  suggestion: string;
}

// Define the structure of our panel props
...
const SECTION_LABELS: Record<string, { title: string; icon: string; color: string }> = {
  strategicFit: { title: "Strategic Fit", icon: "🛡️", color: "border-amber-500" },
  indicatorCompliance: { title: "Indicator Compliance", icon: "📊", color: "border-emerald-500" },
  wordingToneRealism: { title: "Wording & Tone Realism", icon: "✍️", color: "border-indigo-500" },
};
...
export default function AnalyticalPanel({
  ...
  // Dynamic score ring color thresholds mapping back to the PRD [cite: 161]
  const getScoreStyles = (currentScore?: number) => {
    ...
    if (currentScore < 50) return "bg-red-50 text-red-700 border-red-300 animate-pulse-slow";
  ...
  const sectionsData: Record<string, FeedbackItem[]> = {
    strategicFit,
    indicatorCompliance,
    wordingToneRealism,
  };
  ...
  return (
    <div className="w-full md:w-[480px] bg-white rounded-xl border border-slate-200 shadow-sm p-5 flex flex-col h-full overflow-y-auto">
      ...
      {/* Aggregate Index Display Panel [cite: 160] */}
      ...
      {/* Expandable Accordion Queue System [cite: 162] */}
        ...
        {Object.keys(SECTION_LABELS).map((sectionKey) => {
          ...
          return (
            <AccordionSection
              ...
              summary={items} // Directly feeding our array into the summary property matching your error call structure
...
/* ==========================================
   CHILD COMPONENT DEFINITION (FIXES THE ERROR)
   ========================================== */
          ...
          {summary && summary.length > 0 ? (
            summary.map((item, idx) => (
              <div key={idx} className={`py-3.5 first:pt-0 last:pb-0 border-l-4 ${borderColor} pl-3.5 my-2 bg-slate-50/40 rounded-r-lg`}>
                ...
                <span className="text-xs text-emerald-800 block bg-emerald-50/50 p-2.5 rounded-md border border-emerald-100/70 leading-relaxed font-normal">
